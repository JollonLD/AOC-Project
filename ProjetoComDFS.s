...
input_inicio:
    ...
    li $v0, 5
    syscall
    move $s0, $v0  # Salvar local de inicio em $s0
...
input_destino:
    ...
    li $v0, 5
    syscall
    move $s1, $v0  # Salvar local de destino em $s1
...
input_transporte:
    ...
    li $v0, 5
    syscall
    move $s4, $v0  # Salvar meio de transporte em $s4
...
fim_imprimir:
	...
	# Imprimir tempo levado no caminho
	li $v0, 4
	la $a0, saida_tempo
	syscall
